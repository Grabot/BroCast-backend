...
from app.util.util import check_token, get_auth_token, save_other_v1_5


@api_router_v1_5.post("/message/send/other", status_code=200)
async def send_message_other(
    request: Request,
    response: Response,
    ...
    other_data: Optional[UploadFile] = File(default=None),
    ...
    if other_data is not None:
        original_filename = other_data.filename
        ...
        file_name = f"broup_{broup_id}_other_{current_timestamp.strftime('%Y%m%d%H%M%S%f')}___{original_filename}"
        data_type = 7
        
        other_bytes = await other_data.read()

        save_other_v1_5(other_bytes, file_name)
