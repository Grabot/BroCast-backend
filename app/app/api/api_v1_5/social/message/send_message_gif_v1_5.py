...
from app.util.util import check_token, get_auth_token, save_gif_v1_5


@api_router_v1_5.post("/message/send/gif", status_code=200)
async def send_message_gif(
    request: Request,
    response: Response,
    ...
    gif_data: Optional[UploadFile] = File(default=None),
    ...
    if gif_data is not None:
        ...
        file_name = f"broup_{broup_id}_gif_{current_timestamp.strftime('%Y%m%d%H%M%S%f')}"
        data_type = 6
        # Read the contents of the uploaded file
        gif_bytes = await gif_data.read()
        save_gif_v1_5(gif_bytes, file_name)
